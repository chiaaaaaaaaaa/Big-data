...
data_m_agr <- Data_Elections_1998_2024|>
  ...
  summarize( perc_center = round((sum(Votos.válidos[center== 1]/sum(Votos.válidos)))*100,2))
...
election_summary <- Data_Elections_1998_2024 |>
  ...
  reframe(Votos_válidos = sum(Votos.válidos, na.rm = TRUE), #calculate tot votes by municipality
    orientation_center = sum(Votos.válidos[orientation == "center"], na.rm = TRUE)) #votes to the center 
...
#ChatGPT ha detto:
#ChatGPT
#Per rendere il codice più leggibile e più compatto, puoi andare a capo in corrispondenza di ciascun blocco logico degli ifelse. Usa un'indentazione chiara per evidenziare la struttura gerarchica delle condizioni. Ecco una versione migliorata:

right_colors <- ifelse(
  formatted_table_wide$right_diff > 20, "#90EE90",
ifelse(
formatted_table_wide$right_diff > 5 & formatted_table_wide$right_diff <= 20, "#D9F2D1",
ifelse(
formatted_table_wide$right_diff < -20, "#F08080",
ifelse(
formatted_table_wide$right_diff < -5 & formatted_table_wide$right_diff >= -20, "#F9D5D3",
        "#FFFCE8"))))
