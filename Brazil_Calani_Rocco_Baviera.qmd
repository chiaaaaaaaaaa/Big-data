...
y = Average, color = `Nome da Grande Região`))+
    geom_bar(stat = "identity", fill = Palette,  color = "black",
width = 0.5)+
    coord_flip() +
...
ggplot (data = data , aes (x = reorder(`Sigla da Unidade da Federação`,
Average),
...
ggplot (data = data , aes (x = reorder(UF,
`Percentage change of votes to the center`), 
y = `Percentage change of votes to the center`, fill = UF))+
    geom_bar(stat = "identity")+
    coord_flip() +
    scale_y_continuous(labels = label_percent(scale = 1))+
...
ggplot (data = data , aes (x = reorder(UF,
`Average change in orientation index`), 
y =  `Average change in orientation index`, fill = UF))+
    geom_bar(stat = "identity")+
    coord_flip() +
    scale_y_continuous(labels = label_percent(scale = 1))+
  scale_fill_viridis_d(option = "D")+
